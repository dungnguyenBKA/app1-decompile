...
public class AnonymousClass9X {
    ...
    public AnonymousClass9X(RandomAccessFile randomAccessFile, long[] jArr, long j) throws IOException {
    ...
    public static AnonymousClass9X A03(File file) throws IOException {
        ...
        try {
            ...
            randomAccessFile.writeLong((long) AnonymousClass9Y.A03);
            randomAccessFile.write(new byte[(AnonymousClass9Y.A03 * 8)]);
            randomAccessFile.getFD().sync();
            return new AnonymousClass9X(randomAccessFile, new long[AnonymousClass9Y.A03], randomAccessFile.length());
    ...
    public static AnonymousClass9X A04(File file) throws IOException {
        try {
            RandomAccessFile randomAccessFile = new RandomAccessFile(file, A05(HttpStatus.SC_REQUESTED_RANGE_NOT_SATISFIABLE, 3, 41));
            if (file.length() >= A00()) {
                ...
                if (readLong == -360011992771067903L) {
                    ...
                    } else if (randomAccessFile.length() >= A02((int) readLong2)) {
                        long[] jArr = new long[((int) readLong2)];
                        for (int i = 0; i < jArr.length; i++) {
                            jArr[i] = randomAccessFile.readLong();
                        }
                        return new AnonymousClass9X(randomAccessFile, jArr, randomAccessFile.length());
                    } else {
