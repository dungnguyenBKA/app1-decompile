...
public final class AnonymousClass0E {
    ...
    public static List<File> A01(File file) {
        ...
        Collections.sort(asList, new AnonymousClass0D());
        return asList;
    }
    ...
    public static void A05(File file) throws IOException {
        if (!file.delete() || !file.createNewFile()) {
            throw new IOException(A00(47, 30, FacebookMediationAdapter.ERROR_MAPPING_NATIVE_ASSETS) + file);
        }
    }

    public static synchronized void A06(File file) throws IOException {
        synchronized (AnonymousClass0E.class) {
            if (file.exists()) {
                if (!file.isDirectory()) {
                    throw new IOException(A00(FacebookMediationAdapter.ERROR_CREATE_NATIVE_AD_FROM_BID_PAYLOAD, 5, 68) + file + A00(0, 18, 82));
                }
            } else if (!file.mkdirs()) {
