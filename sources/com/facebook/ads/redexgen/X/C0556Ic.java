...
public final class C0556Ic {
    ...
    public static int A09(byte[] bArr, int i, int i2, int i3) {
        while (i < i2) {
            i3 = (i3 << 8) ^ A0A[((i3 >>> 24) ^ (bArr[i] & 255)) & 255];
            i++;
        }
        return i3;
    }
    ...
    /* JADX INFO: Multiple debug info for r6v0 long: [D('multiplicationFactor' long), D('divisionFactor' long), D('multiplicationFactor' double)] */
    public static long A0F(long divisionFactor, long j, long j2) {
        if (j2 >= j && j2 % j == 0) {
            return divisionFactor / (j2 / j);
        }
        if (j2 < j && j % j2 == 0) {
            return divisionFactor * (j / j2);
        }
        double d = (double) j;
        String[] strArr = A01;
        if (strArr[1].charAt(11) != strArr[2].charAt(11)) {
        ...
        return (long) (((double) divisionFactor) * (d / ((double) j2)));
    }
    ...
    public static long A0I(long maxPositionUs, C0370Ab ab, long j, long j2) {
        boolean z;
        if (C0370Ab.A04.equals(ab)) {
            return maxPositionUs;
        }
        long A0H = A0H(maxPositionUs, ab.A01, Long.MIN_VALUE);
        long A0G = A0G(maxPositionUs, ab.A00, Long.MAX_VALUE);
        boolean z2 = true;
        if (A0H > j || j > A0G) {
            z = false;
        } else {
    ...
    public static ExecutorService A0T(String str) {
        return Executors.newSingleThreadExecutor(new ThreadFactoryC0555Ib(str));
    }
    ...
    /* JADX INFO: Multiple debug info for r7v0 long[]: [D('multiplicationFactor' long), D('divisionFactor' long), D('multiplicationFactor' double)] */
