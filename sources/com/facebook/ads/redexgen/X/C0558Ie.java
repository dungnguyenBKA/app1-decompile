...
public final class C0558Ie {
    ...
    public C0558Ie(List<byte[]> initializationData, int i, int i2, int i3, float f) {
    ...
    public static C0558Ie A00(IM im) throws AJ {
        try {
            im.A0a(4);
            int A0F = (im.A0F() & 3) + 1;
            if (A0F != 3) {
                ArrayList arrayList = new ArrayList();
                int numSequenceParameterSets = im.A0F() & 31;
                for (int j = 0; j < numSequenceParameterSets; j++) {
                    arrayList.add(A04(im));
                }
                int A0F2 = im.A0F();
                ...
                if (A06[4].length() != 27) {
                    ...
                    if (numSequenceParameterSets > 0) {
                        IH A062 = II.A06((byte[]) arrayList.get(0), A0F, ((byte[]) arrayList.get(0)).length);
                        i = A062.A06;
                        i2 = A062.A02;
                        f = A062.A00;
                    }
                    return new C0558Ie(arrayList, A0F, i, i2, f);
                }
                throw new RuntimeException();
            }
            throw new IllegalStateException();
        } catch (ArrayIndexOutOfBoundsException e) {
            throw new AJ(A01(0, 24, 119), e);
        }
    }

    public static byte[] A04(IM im) {
        int offset = im.A0J();
        int A07 = im.A07();
        im.A0a(offset);
        return I1.A09(im.A00, A07, offset);
    }
}
