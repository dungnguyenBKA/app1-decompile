package com.google.android.gms.internal.gtm;

import java.util.List;

/* access modifiers changed from: package-private */
public interface zzww {
    void zzA(List<Double> list);

    void zzB(List<Integer> list);

    void zzC(List<Integer> list);

    void zzD(List<Long> list);

    void zzE(List<Float> list);

    @Deprecated
    <T> void zzF(List<T> list, zzwx<T> zzwx, zzuj zzuj);

    void zzG(List<Integer> list);

    void zzH(List<Long> list);

    <T> void zzI(List<T> list, zzwx<T> zzwx, zzuj zzuj);

    void zzJ(List<Integer> list);

    void zzK(List<Long> list);

    void zzL(List<Integer> list);

    void zzM(List<Long> list);

    void zzN(List<String> list);

    void zzP(List<String> list);

    void zzQ(List<Integer> list);

    void zzR(List<Long> list);

    boolean zzS();

    boolean zzT();

    double zza();

    float zzb();

    int zzc();

    int zzd();

    int zze();

    int zzf();

    int zzg();

    int zzh();

    int zzi();

    int zzj();

    long zzk();

    long zzl();

    long zzm();

    long zzn();

    long zzo();

    zztd zzq();

    @Deprecated
    <T> T zzr(Class<T> cls, zzuj zzuj);

    @Deprecated
    <T> T zzs(zzwx<T> zzwx, zzuj zzuj);

    <T> T zzt(Class<T> cls, zzuj zzuj);

    <T> T zzu(zzwx<T> zzwx, zzuj zzuj);

    String zzv();

    String zzx();

    void zzy(List<Boolean> list);

    void zzz(List<zztd> list);
}
