...
public class RouteSpecificPool {
    ...
    public RouteSpecificPool(HttpRoute httpRoute, int i) {
        ...
        this.connPerRoute = new ConnPerRoute() {
            /* class org.apache.http.impl.conn.tsccm.RouteSpecificPool.AnonymousClass1 */
    ...
    public void createdEntry(BasicPoolEntry basicPoolEntry) {
        ...
        StringBuilder q = ic.q("Entry not planned for this pool.\npool: ");
        q.append(this.route);
        q.append("\nplan: ");
        q.append(basicPoolEntry.getPlannedRoute());
    ...
    public void freeEntry(BasicPoolEntry basicPoolEntry) {
        int i = this.numEntries;
        if (i < 1) {
            StringBuilder q = ic.q("No entry created for this pool. ");
            q.append(this.route);
            throw new IllegalStateException(q.toString());
        } else if (i > this.freeEntries.size()) {
            this.freeEntries.add(basicPoolEntry);
        } else {
            StringBuilder q2 = ic.q("No entry allocated from this pool. ");
            q2.append(this.route);
